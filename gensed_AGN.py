...
class gensed_AGN(gensed_base):
    def __init__(self, PATH_VERSION, OPTMASK, ARGLIST, HOST_PARAM_NAMES):
        ...
        self.parse_param(arglist=ARGLIST)
    ...
    def parse_param(self, arglist):
        ...
        self.ranseed = int(args.pop('RANSEED', 0))
        self.cl_prob = float(args.pop('cl_prob', 0.0))
        if len(args) != 0:
            raise ValueError(f'ARGLIST has unknown arguments: {" ".join(args)}')
